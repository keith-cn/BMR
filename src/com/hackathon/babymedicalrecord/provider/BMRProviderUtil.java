...
public class BMRProviderUtil {
    ...
    public static final class User {
        ...
        private static final String[] READ_ITEM_PROJECTION = new String[] { BMR.User._ID,
                BMR.User.COLUMN_NAME_ACCOUNT, BMR.User.COLUMN_NAME_PASSWORD };

        public static Uri insertItem(Context context, String accountName, String password) {
        ...
        public static boolean deleteItemById(Context context, long id) {
            int result = context.getContentResolver().delete(UserUri, BMR.User._ID + "=?",
        ...
        public static boolean deleteItemByAccountName(Context context, String account) {
            int result = context.getContentResolver().delete(UserUri,
                    BMR.User.COLUMN_NAME_ACCOUNT + "=?", new String[] { account });
            if (result > 0)
                return true;
            return false;
        }
        ...
        public static boolean deleteAllItems(Context context) {
            int result = context.getContentResolver().delete(UserUri, null, null);
            if (result > 0)
                return true;
            return false;
        }
        ...
        public static UserItem getItemById(Context context, long id) {
            ...
            Cursor c = context.getContentResolver().query(UserUri, READ_ITEM_PROJECTION, selection,
        ...
        public static ArrayList<UserItem> getAllItems(Context context) {
            Cursor c = context.getContentResolver().query(UserUri, READ_ITEM_PROJECTION, null,
                    null, null);
            if (c == null) {
            ...
            try {
                if (c.getCount() > 0 && c.moveToFirst()) {
                    ...
                    while (c.moveToNext()) {
                        ...
                        item2.id = c.getLong(idIndex);
                        item2.account = c.getString(accountIndex);
                        item2.password = c.getString(passwordIndex);
                        list.add(item2);
                    }
        ...
        public static CursorLoader getCursorLoaderForAllItems(Context context) {
            CursorLoader loader = new CursorLoader(context, UserUri, READ_ITEM_PROJECTION, null,
                    null, null);
            return loader;
        }

        public static Cursor getCursorForAllItems(Context context) {
            Cursor result = context.getContentResolver().query(UserUri, READ_ITEM_PROJECTION, null,
                    null, null);
            return result;
        }
    }
